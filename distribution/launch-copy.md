...
All new copy below is pinned to `@backbond/agent-scan@0.6.3`.

Publish only the initial post below before the five external attempts on the pinned 0.6.3 command are recorded. The remaining channel variants are held drafts, not a posting calendar.

## Two-sentence canonical announcement

Before attaching a new MCP tool, vet its `tools/list` metadata locally with `npx -y @backbond/agent-scan@0.6.3 vet-tools --stdin`. A forced-invocation description can produce `BB013`; the deterministic gate returns block, review, or no blocking finding without executing the tool or uploading the manifest. Exit 0 is not proof of runtime safety or insurance.

## Founder social post

An agent should not attach a third-party MCP tool just because its description sounds useful.

We released `@backbond/agent-scan@0.6.3`, a free local pre-attachment gate for MCP and AI-agent tools. Pipe in the captured `tools/list` response and it checks dangerous capabilities, tool-description manipulation, confusable tool names, and unsafe tool combinations.
