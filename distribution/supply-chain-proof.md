...
For version `0.6.3`, a reviewer can reproduce both shipped artifacts from the public tag:
...
Compare both computed digests with the two `.sha256` files on the official `v0.6.3` source release. A mismatch is a stop condition. The page must not hard-code hashes before the protected release has produced and independently verified the final artifacts.
