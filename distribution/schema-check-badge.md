...
Use **BackBond Schema Check**, never verification or certification language. A passing workflow means that the committed manifest produced `no_blocking_finding` under the pinned static pre-attachment profile. It is not runtime verification, insurance coverage, or proof that the deployed server matches the manifest.
