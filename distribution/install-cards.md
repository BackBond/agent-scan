...
```powershell
if (Test-Path .\tools-list.json) { Get-Content -Raw .\tools-list.json | npx -y @backbond/agent-scan@0.6.0 vet-tools --stdin } else { Write-Error "Save a real MCP tools/list response as tools-list.json first." }
```
