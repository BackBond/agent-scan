...
- Exit 3 (`review`): a medium finding or incomplete/ambiguous evidence requires operator review. Stop unknown and obtain a better manifest or human decision.
...
If `tools-list.json` does not exist, do not invent it and do not report a decision. Run `npx -y @backbond/agent-scan@0.6.1 scan` for bounded local discovery and follow its exact next action.

Never post or upload raw tool manifests, prompts, traces, configurations, full JSON reports, or path-bearing receipts. Share only compact `vet-tools` text after redaction, identity-free `vet-tools --summary-only` output for aggregate work, or a compact `--record-public` card.
```
