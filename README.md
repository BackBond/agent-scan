...
BackBond Agent Scan is published as the exact scoped package `@backbond/agent-scan@0.5.9`. Static only: it inspects tool metadata and agent configuration already on your machine. It does not run tools, probe a live agent, upload traces, contact a hosted service, or execute a second binary.
...
The installed file must be `.agents/skills/agent-scan/SKILL.md`. It keeps commands pinned to `0.5.9`, treats coverage gaps as unknown rather than safe, and tells agents never to post raw manifests, traces, prompts, JSON reports, or path-bearing receipts.
...
The standalone asset above is the simplest offline path. An operator may instead download `backbond-agent-scan-0.5.9.tgz` and its `.sha256` file from the official `v0.5.9` source release and transfer both through the organization's trusted software path. Verify the transferred bytes at the destination immediately before running them:

```bash
sha256sum --check backbond-agent-scan-0.5.9.tgz.sha256
npm exec --yes --offline --package=./backbond-agent-scan-0.5.9.tgz -- agent-scan scan
```

On Windows PowerShell, compare `(Get-FileHash .\backbond-agent-scan-0.5.9.tgz -Algorithm SHA256).Hash` with the first value in the `.sha256` file and stop on any mismatch.
...
Agent frameworks can use the scanner today by exporting their runtime tool list to one of the generic formats or stdin. Version 0.5.9 does not execute or import arbitrary framework code to discover tools; framework-specific code extractors need dedicated isolation and belong after this deterministic intake.
