...
Four anonymized non-BackBond examples in [`fixtures/wild`](fixtures/wild) prove that findings fire on MCP, tool-description poisoning, wildcard sandbox scopes, and trusted-tool configuration files rather than only on canonical fixtures. The 18 cases in [`fixtures/corpus-regression`](fixtures/corpus-regression)—ten synthetic boundaries plus eight redacted BB001 cases derived from the saved-output delta audit—lock the BLOCK/REVIEW/no-blocking distinctions found during the 2026-08-31 corpus review. They retain no server names or URLs. See [the corpus regression and publication boundary](docs/CORPUS-REGRESSION.md).
