...
BackBond Agent Scan is published as the exact scoped package `@backbond/agent-scan@0.5.14`. Static only: it inspects tool metadata and agent configuration already on your machine. It does not run tools, probe a live agent, upload traces, contact a hosted service, or execute a second binary.
...
```bash
curl -fLO https://github.com/BackBond/agent-scan/releases/download/v0.5.14/agent-scan.cjs
curl -fLO https://github.com/BackBond/agent-scan/releases/download/v0.5.14/agent-scan.cjs.sha256
...
```bash
git clone --depth 1 --branch v0.5.14 https://github.com/BackBond/agent-scan.git
cd agent-scan
npm pack
sha256sum backbond-agent-scan-0.5.14.tgz
node scripts/build-standalone.js agent-scan.cjs
sha256sum agent-scan.cjs
```

Compare both computed digests with the `.sha256` files on the official `v0.5.14` release. A mismatch is a stop condition. Provenance describes the protected build; byte-for-byte reproduction independently connects the public tag to the released artifacts. Neither is a runtime attestation or insurance decision.
...
```bash
npx -y @backbond/agent-scan@0.5.14 scan
npx -y @backbond/agent-scan@0.5.14 vet-tools --stdin < tools-list.json
```
...
The standalone asset above is the simplest offline path. An operator may instead download `backbond-agent-scan-0.5.14.tgz` and its `.sha256` file from the official `v0.5.14` source release and transfer both through the organization's trusted software path. Verify the transferred bytes at the destination immediately before running them:

```bash
sha256sum --check backbond-agent-scan-0.5.14.tgz.sha256
npm exec --yes --offline --package=./backbond-agent-scan-0.5.14.tgz -- agent-scan scan
```

On Windows PowerShell, compare `(Get-FileHash .\backbond-agent-scan-0.5.14.tgz -Algorithm SHA256).Hash` with the first value in the `.sha256` file and stop on any mismatch.
...
```bash
npx -y @backbond/agent-scan@0.5.14 scan --stdin --require-coverage < tools-list.json
```
...
Agent frameworks can use the scanner today by exporting their runtime tool list to one of the generic formats or stdin. Version 0.5.14 does not execute or import arbitrary framework code to discover tools; framework-specific code extractors need dedicated isolation and belong after this deterministic intake.
...
Each finding includes a stable ID, severity, affected tools, evidence pointers, evidence quality, an immediate `Stop` instruction, and remediation. See [docs/RULES.md](docs/RULES.md) or the stable web permalinks under [`/agent-scan/rules/#BB001`](https://backbond.ai/agent-scan/rules/#BB001).
...
In the Action's strict `vet-tools` mode, set an optional repository-relative `sarif-path` to write the same named findings with stable rule help links. The Action does not upload the file; any upload or code-scanning step remains a separate, explicit workflow decision.
...
steps:
  ...
  - id: agent-scan
    uses: BackBond/agent-scan@v0.5.14
    with:
...
Omit `sarif-path` when the repository does not want a file containing finding locations. The job summary remains compact and excludes raw tool names.
