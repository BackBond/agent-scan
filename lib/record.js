...
function createPublicScanRecord(scan, receipt, options = {}) {
  ...
  const payload = {
    ...
    assurance: {
      ...
      statement: 'Local scan record only. Not a safety certificate or BackBond attestation.',
    },
...
function verifyPublicScanRecord(record) {
  if (!record || record.protocol !== RECORD_PROTOCOL || !record.integrity) return false;
