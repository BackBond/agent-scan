...
function scanEvidence(evidence, options = {}) {
  ...
  return {
    ...
    status: evaluated.findings.length ? 'findings' : 'no_findings',
    summary: summary(evaluated.findings),
    findings: evaluated.findings,
    coverage: { status: gaps.length ? 'partial' : 'complete', gaps },
...
function scannerContract() {
  return {
    ...
    ruleset: { version: RULESET_VERSION, sha256: RULESET_DIGEST },
    supported_inputs: {
      tool_schema: ['backbond-tool-schema/v1', 'openai-function-tools/v1', 'anthropic-tools/v1', 'mcp-tools-list/v1'],
      permissions: ['backbond-permissions/v1'],
      trace: ['backbond-trace/v1'],
