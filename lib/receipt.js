...
function attachArtifactDigests(findings, inputs) {
  const byKind = new Map(inputs.map(input => [input.kind, input.sha256]));
  return findings.map(finding => ({
    ...finding,
    evidence_refs: finding.evidence_refs.map(item => ({ ...item, artifact_sha256: byKind.get(item.artifact_kind) || null })),
  }));
}
