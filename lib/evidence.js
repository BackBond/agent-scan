...
function hasPositiveExecutionDescription(description) {
  const withoutNegatedPhrases = description.replace(
    /\b(?:does not|doesn't|do not|don't|never|cannot|can't|without)\s+(?:actually\s+)?(?:exec(?:ute|uting)?|run(?:ning)?|launch(?:ing)?|invok(?:e|ing)|evaluat(?:e|ing))\b.{0,50}?(?=\s*(?:,\s*)?\b(?:and|but|however|yet|while|although)\b|[.;!?]|$)/g,
    ' ',
  );
  ...
  const passive = /(?:^|[.!?;]\s*)\b(?:shell|bash|terminal|commands?|code|scripts?|powershell|expressions?|sql|subprocess)\b.{0,30}\b(?:is|are|can be|will be|may be|must be)\s+(?:exec(?:ute|uted)?|run|launched|invoked|evaluated)\b/;
  return active.test(withoutNegatedPhrases) || passive.test(withoutNegatedPhrases);
}
