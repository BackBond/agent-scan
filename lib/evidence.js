...
function hasInputSchema(tool) {
  return ['parameters', 'input_schema', 'inputSchema', 'schema'].some(key => Object.prototype.hasOwnProperty.call(tool, key)
    && tool[key] !== null && typeof tool[key] === 'object' && !Array.isArray(tool[key]));
}
...
function mergeToolFacts(sources, globalInputTrust) {
  ...
  return [...grouped.entries()].map(([name, entries]) => {
    ...
    return {
      ...
      input_schema_observed: entries.some(entry => entry.input_schema_observed === true),
