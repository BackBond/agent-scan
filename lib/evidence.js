...
function executionDescriptionSignals(description) {
  ...
  const active = /\b(?:exec(?:ute|utes|uting)?|run(?:s|ning)?|launch(?:es|ing)?|invoke(?:s|d|ing)?|evaluate(?:s|d|ing)?)\b.{0,50}\b(?:shell|bash|terminal|commands?|code|scripts?|powershell|expressions?|sql|subprocess)\b/;
  const passive = /(?:^|[.!?;]\s*)(?:(?:the|an?)\s+)?(?:(?:user[- ]supplied|model[- ]generated|supplied|arbitrary|raw|input)\s+){0,3}\b(?:shell|bash|terminal|commands?|code|scripts?|powershell|expressions?|sql|subprocess)\b.{0,30}\b(?:is|are|can be|will be|may be|must be)\s+(?:exec(?:ute|uted)?|run|launched|invoked|evaluated)\b/;
  return { active: active.test(withoutNegatedPhrases), passive: passive.test(withoutNegatedPhrases) };
}
...
function inferCapabilities(tool) {
  ...
  const passiveDocumentationOnly = documentationIdentity && documentationDescription
    && executionDescription.passive && !executionDescription.active && !executionField;
  const matches = [
  ...
  if (executionField
    ...
    || ((executionDescription.active || executionDescription.passive) && !passiveDocumentationOnly)) {
...
function toolDescriptionRisks(tool, artifact, toolPointer) {
  ...
  const forcedInvocationPatterns = [
    /\b(?:always|must)\b.{0,36}\b(?:call|use|invoke|select|choose)\b.{0,36}\b(?:this|the)\s+tool\b/,
    /\b(?:you|agent|assistant|model)\b.{0,24}\b(?:are\s+required|have|need)\s+to\b.{0,30}\b(?:call|use|invoke|select|choose)\b.{0,30}\b(?:this|the)\s+tool\b/,
    /\b(?:before|prior to)\b.{0,60}\b(?:answer(?:ing)?|respond(?:ing)?|reply(?:ing)?|handling)\b.{0,60}\b(?:any|every|the|a)?\s*(?:question|request|prompt|task)?\b.{0,60}\b(?:call|use|invoke|select|choose)\b.{0,30}\b(?:this|the)\s+tool\b/,
    /\b(?:call|use|invoke|select|choose)\b.{0,30}\b(?:this|the)\s+tool\b.{0,60}\b(?:before|prior to)\b.{0,60}\b(?:answer(?:ing)?|respond(?:ing)?|reply(?:ing)?|handling)\b/,
    /\b(?:this|the)\s+tool\b.{0,24}\b(?:must|always\s+(?:has|needs)\s+to|is\s+required\s+to)\b.{0,16}\b(?:be\s+)?(?:called|used|invoked|selected|chosen)\b/,
    /\bprefer\b.{0,30}\b(?:this|the)\s+tool\b.{0,40}\b(?:over|instead of|rather than)\b/,
    /\b(?:do not|don't|never|avoid)\b.{0,30}\b(?:call|use|invoke|select|choose)\b.{0,30}\b(?:other|another|competing)\s+tools?\b/,
  ];
  const fields = [
  ...
  for (const [field, value] of fields) {
    ...
    if (toolLevelMetadata && forcedInvocationPatterns.some(pattern => pattern.test(text))) {
      risks.push({ id: 'prompt_forced_invocation', reason: 'tool description attempts to control tool selection or force invocation', ref: ref(artifact, pointer) });
    }
  }
