...
function executionDescriptionSignals(description) {
  const withoutNegatedPhrases = description.replace(
    /\b(?:does not|doesn't|do not|don't|never|cannot|can't|without)\s+(?:actually\s+)?(?:exec(?:ute|utes|uting)?|run(?:s|ning)?|launch(?:es|ing)?|invoke(?:s|d|ing)?|evaluate(?:s|d|ing)?)\b.{0,50}?(?=\s*,|\s+\b(?:and|but|however|yet|while|although)\b|[.;!?]|$)/g,
    ' ',
  );
  const active = /\b(?:exec(?:ute|utes|uting)?|run(?:s|ning)?|launch(?:es|ing)?|invoke(?:s|d|ing)?)\b.{0,50}\b(?:shell|bash|terminal|commands?|code|scripts?|powershell|subprocess)\b/;
  const passive = /(?:^|[.!?;]\s*)(?:(?:the|an?)\s+)?(?:(?:user[- ]supplied|model[- ]generated|supplied|arbitrary|raw|input)\s+){0,3}\b(?:shell|bash|terminal|commands?|code|scripts?|powershell|subprocess)\b.{0,30}\b(?:is|are|can be|will be|may be|must be)\s+(?:exec(?:ute|uted)?|run|launched|invoked)\b/;
...
function executionInputField(name, node, toolContext) {
  ...
  const queryLike = name === 'query';
  const fieldContext = normalizedWords(`${typeof node.description === 'string' ? node.description : ''} ${toolContext}`);
  const commandContext = /\b(?:run|runner|exec(?:ute|ution)?|shell|terminal|bash|powershell|subprocess|interpreter|source code|script|program|command)\b/.test(fieldContext);
  const expressionContext = /\b(?:eval|evaluator|interpreter|javascript|python|expression engine)\b/.test(fieldContext);
  const databaseContext = /\b(?:database|db|postgres(?:ql)?|mysql|sqlite|sql server|query engine|sql runner|execute sql|run sql)\b/.test(fieldContext);
  return strong || (commandLike && commandContext) || (expressionLike && expressionContext) || ((sqlLike || queryLike) && databaseContext);
}
...
function inferCapabilities(tool) {
  ...
  const executionDescription = executionDescriptionSignals(description);
  const queryExecutionContext = /\b(?:database|db|postgres(?:ql)?|mysql|sqlite|sql server|query engine|sql runner|interpreter)\b/.test(toolContext)
    || /\b(?:run|execute|eval)\b.{0,30}\b(?:sql|query|expression)\b/.test(identity);
  const documentationIdentity = /\b(doc|docs|documentation|handbook|reference|catalog|list|explain|describe|lint|analy[sz]e|parse|inspect|preview)\b/.test(identity);
  const documentationDescription = /\b(document(?:ation|ed)?|handbook|reference|catalog|guide|explains?|describes?|examples?|read[ -]only|prose(?: only)?|text only)\b/.test(description);
  ...
  if (executionField
    || (executionIdentity && !(documentationIdentity && documentationDescription && !executionDescription.active) && !passiveDocumentationOnly)
    || ((executionDescription.active || executionDescription.passive || (executionDescription.query && queryExecutionContext)) && !passiveDocumentationOnly)) {
...
function semanticRisks(tool, artifact, toolPointer) {
  ...
  while (pending.length) {
    ...
    if ((node.type === 'string' || node.type === undefined) && !constrained) {
      ...
      const explicitInterpreter = /\b(?:eval|source code|shell command|command text)\b/.test(label);
      const queryInterpreterContext = /\b(?:database|db|postgres(?:ql)?|mysql|sqlite|sql server|query engine|sql runner|expression engine|interpreter)\b/.test(toolContext);
      const queryExpression = /\b(?:sql|sql query|expression|query language|where clause|filter expression)\b/.test(label)
        || (normalizedWords(propertyName).trim() === 'query' && queryInterpreterContext);
      if (interpreterField
        || (explicitInterpreter && /\b(?:run|exec(?:ute|ution)?|shell|interpreter|eval)\b/.test(toolContext))
        || (queryExpression && queryInterpreterContext)) {
        ...
        risks.push({ id: 'ambiguous_query_expression', reason: `${propertyName || 'input'} accepts query or expression text whose execution semantics are not observable`, ref: ref(artifact, pointer) });
      }
    }
    ...
    if (destinationClass) {
      ...
      risks.push({ id: destinationClass, reason, ref: ref(artifact, pointer) });
    }
