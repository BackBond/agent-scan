...
function hasPositiveExecutionDescription(description) {
  ...
  return /(?:^|[.!?;]\s*|\b(?:this\s+)?(?:tool|server|function)\s+|\bit\s+|\b(?:can|will)\s+)(?:can\s+|will\s+)?(?:exec(?:ute|utes|uting)?|run(?:s|ning)?|launch(?:es|ing)?|invoke(?:s|d|ing)?|evaluate(?:s|d|ing)?)\b.{0,50}\b(?:shell|bash|terminal|commands?|code|scripts?|powershell|expressions?|sql|subprocess)\b/.test(withoutNegatedPhrases);
}
...
function inferCapabilities(tool) {
  ...
  const executionField = schemaFields.some(name => /^(cmd|command|python|code|script|shell|sql|expression|eval|sql query|shell command|command text|source code|python code)$/.test(name));
...
function semanticRisks(tool, artifact, toolPointer) {
  ...
  function walk(node, pointer, propertyName = '') {
    ...
    if ((node.type === 'string' || node.type === undefined) && !constrained) {
      const interpreterField = /^(cmd|command|python|code|script|shell|sql|expression|eval|sql query|shell command|command text|source code|python code)$/i.test(normalizedWords(propertyName));
