...
function schemaFieldNames(value, depth = 0) {
  if (!value || typeof value !== 'object' || depth > 5) return [];
  const names = [];
  if (value.properties && typeof value.properties === 'object') {
    for (const [name, child] of Object.entries(value.properties)) {
      names.push(normalizedWords(name));
      names.push(...schemaFieldNames(child, depth + 1));
    }
  }
  if (value.items) names.push(...schemaFieldNames(value.items, depth + 1));
  for (const keyword of ['anyOf', 'oneOf', 'allOf']) {
    if (Array.isArray(value[keyword])) value[keyword].forEach(child => names.push(...schemaFieldNames(child, depth + 1)));
  }
  return names;
}
...
function inferCapabilities(tool) {
  ...
  const schemaFields = schemaFieldNames(inputSchema(tool));
  const executionIdentity = /\b(shell|bash|terminal|exec(?:ute)?|subprocess|powershell|run code|eval)\b/.test(identity);
  const executionField = schemaFields.some(name => /^(cmd|command|python|code|script|shell|sql|query|expression|eval|sql query|shell command|command text|source code|python code)$/.test(name));
  const executionDescription = /\b(shell|bash|terminal|exec(?:ute|ution)?|command|subprocess|powershell|run code|source code|eval(?:uate)? expression)\b/.test(description);
  const executionNegated = /\b(?:no|without)\s+(?:actual\s+)?(?:shell\s+|command\s+|code\s+)?execution\b(?!\s+(?:result|output|status|details))|\b(?:does not|doesn't|never)\s+(?:execute|run)\b|\bread[- ]only\b.{0,80}\b(?:document(?:ation|ed)?|handbook|reference|list)\b/.test(description);
  ...
  if (executionField || (executionIdentity && !(documentationIdentity && executionNegated)) || (executionDescription && !executionNegated)) {
...
function semanticRisks(tool, artifact, toolPointer) {
  ...
  function walk(node, pointer, propertyName = '') {
    ...
    const usefulPattern = typeof node.pattern === 'string' && !['.*', '^.*$', '.+'].includes(node.pattern.trim());
    const constrained = Array.isArray(node.enum) || usefulPattern;
    if ((node.type === 'string' || node.type === undefined) && !constrained) {
      const interpreterField = /^(cmd|command|python|code|script|shell|sql|query|expression|eval|sql query|shell command|command text|source code|python code)$/i.test(normalizedWords(propertyName));
...
function normalizeClaudeSettings(document, artifact) {
  ...
  for (const [field, approval] of [['allow', 'none'], ['ask', 'enforced']]) {
    ...
    controls[field].forEach((rule, index) => {
      ...
      tools.push(normalizeTool({ name, description: rule, inputSchema: { type: 'object' } }, index, artifact, `/permissions/${field}`, artifact.dialect, { pointerOverride: pointer, approval }));
      const bodyMatch = rule.trim().match(/^[^(]+\((.*)\)$/);
      const body = bodyMatch ? bodyMatch[1].trim() : '';
      const normalizedName = name.toLowerCase();
      const globalPath = scopeIsWildcard(body) || /^(?:\/{1,2}\*{1,2}|[a-z]:[\\/]\*{1,2})$/i.test(body);
      if (normalizedName === 'bash' && scopeIsWildcard(body)) permission.wildcards.push({ domain: 'subprocess', field: 'allow', provenance: 'derived', ref: ref(artifact, pointer) });
      ...
      if (normalizedName === 'webfetch' && /^(?:domain\s*:\s*)?\*$/i.test(body)) {
...
function normalizeAgentConfig(document, artifact) {
  ...
  if (network.allowedDomains !== undefined) permission.network_scope_observed = true;
  ...
  for (const container of containers) {
    for (const [serverName, server] of Object.entries(container.servers)) {
      ...
      const roleText = `${serverName} ${server.command || ''} ${roleArgs.join(' ')}`;
