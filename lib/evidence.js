...
const OPAQUE_SCHEMA_BRANCHES = new Set([
  ...
  'prefixItems', 'unevaluatedProperties', 'contentSchema',
]);
...
function toolDescriptionRisks(tool, artifact, toolPointer) {
  ...
  function addSchemaMetadata(node, pointer, depth = 0) {
    ...
    if (node.properties && typeof node.properties === 'object') {
      for (const [name, child] of Object.entries(node.properties)) addSchemaMetadata(child, `${pointer}/properties/${escapePointer(name)}`, depth + 1);
    }
    if (node.items) addSchemaMetadata(node.items, `${pointer}/items`, depth + 1);
    for (const keyword of ['anyOf', 'oneOf', 'allOf']) {
      if (Array.isArray(node[keyword])) node[keyword].forEach((child, index) => addSchemaMetadata(child, `${pointer}/${keyword}/${index}`, depth + 1));
    }
  }
