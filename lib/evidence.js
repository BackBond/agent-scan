...
function inferCapabilities(tool) {
  ...
  const positiveExecutionClause = description.split(/(?:[.;!?]\s*|\b(?:but|however)\b)/).some(clause => {
    const positive = /\b(?:exec(?:ute|utes|uting)?|runs?|launch(?:es)?|invokes?|evaluates?)\b.{0,50}\b(?:shell|bash|terminal|commands?|code|scripts?|powershell|expressions?)\b|\b(?:shell|bash|terminal|commands?|code|scripts?|powershell|expressions?)\b.{0,50}\b(?:exec(?:ute|utes|ution)?|runs?|launch(?:es)?|invokes?|evaluates?)\b/.test(clause);
    const negated = /\b(?:does not|doesn't|do not|don't|never|without|no)\b.{0,40}\b(?:execute|run|launch|invoke|evaluate|execution)\b/.test(clause);
  ...
  if (executionField || (executionIdentity && !(documentationIdentity && executionNegated)) || (executionDescription && (!executionNegated || positiveExecutionClause))) {
