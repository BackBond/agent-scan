...
function schemaFields(value, depth = 0) {
  if (!value || typeof value !== 'object' || depth > 5) return [];
  const fields = [];
  if (value.properties && typeof value.properties === 'object') {
    for (const [name, child] of Object.entries(value.properties)) {
      fields.push({ name: normalizedWords(name), node: child });
      fields.push(...schemaFields(child, depth + 1));
    }
  }
  if (value.items) fields.push(...schemaFields(value.items, depth + 1));
  for (const keyword of ['anyOf', 'oneOf', 'allOf']) {
    if (Array.isArray(value[keyword])) value[keyword].forEach(child => fields.push(...schemaFields(child, depth + 1)));
  }
  return fields;
}
...
function inferCapabilities(tool) {
  ...
  const fields = schemaFields(inputSchema(tool));
  ...
  const executionField = fields.some(field => executionInputField(field.name, field.node, toolContext));
