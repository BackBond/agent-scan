...
function toolDescriptionRisks(tool, artifact, toolPointer) {
  ...
  const forcedInvocationPatterns = [
    /\b(?:always|must|required to|have to|need to)\b.{0,36}\b(?:call|use|invoke|select|choose)\b.{0,36}\b(?:this|the)\s+tool\b/,
  ...
  for (const [field, value] of fields) {
    ...
    if (forcedInvocationPatterns.some(pattern => pattern.test(text))) {
