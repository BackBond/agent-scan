...
function semanticRisks(tool, artifact, toolPointer) {
  ...
  if (analysisIncomplete) {
    risks.push({
      ...
      reason: `input schema exceeds the local analysis budget (${MAX_SCHEMA_ANALYSIS_DEPTH} levels or ${MAX_SCHEMA_ANALYSIS_NODES} nodes)`,
...
function sensitiveSolicitationSignal(clause) {
  ...
  if (preventsTransfer.test(clause) || statusOnly.test(clause)) return false;
  ...
  return direct.test(clause) || addressed.test(clause) || askUser.test(clause) || requiredPassive.test(clause);
}
...
function toolDescriptionRisks(tool, artifact, toolPointer) {
  ...
  for (const [field, value] of fields) {
    ...
    const sensitiveRequest = clauses.some(sensitiveSolicitationSignal);
    ...
    if (sensitiveRequest) {
      risks.push({ id: 'prompt_sensitive_data_request', reason: 'tool description directly solicits sensitive data for a result or transmission', ref: ref(artifact, pointer), metadata_sha256: metadataSha256 });
    }
    if (forcedInvocation) {
