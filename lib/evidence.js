...
function inferCapabilities(tool) {
  ...
  const matches = [
    ['code_execution', /\b(shell|bash|terminal|exec(?:ute)?|command|subprocess|powershell|run code|source code|eval(?:uate)? expression)\b/],
    ['secret_read', /\b(secret|credential|password|token|keychain|vault|api key|environment variable)\b/],
...
function semanticRisks(tool, artifact, toolPointer) {
  ...
  function walk(node, pointer, propertyName = '') {
    ...
    if ((node.type === 'string' || node.type === undefined) && !constrained) {
      if (/\b(sql|sql query|eval|expression|source code|script|shell command|command text)\b/.test(label)) {
...
function normalizeTool(tool, index, artifact, basePointer, dialect, options = {}) {
  ...
  const risks = semanticRisks(tool, artifact, toolPointer);
  return {
    name: tool.name,
    dialect,
    capabilities,
...
function normalizeClaudeSettings(document, artifact) {
  if (artifact.adapter !== 'claude-code' || !document || typeof document !== 'object' || (!document.permissions && !document.sandbox)) return null;
...
function normalizeAgentConfig(document, artifact) {
  ...
  for (const container of containers) {
    for (const [serverName, server] of Object.entries(container.servers)) {
      ...
      const identityText = `${serverName} ${server.command || ''} ${(server.args || []).join(' ')}`.toLowerCase();
      const forced = [];
      if (/filesystem|file-system/.test(identityText)) forced.push('filesystem_access');
      if (/postgres|mysql|sqlite|database/.test(identityText)) forced.push('persistent_write');
      ...
      if (forced.includes('filesystem_access') && (server.args || []).some(scopeIsWildcard)) addWildcard('filesystem', 'read', `${pointer}/args`);
    }
  }
