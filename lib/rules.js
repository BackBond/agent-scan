...
const RULESET_VERSION = 'backbond-local-rules/1.3.0';
...
const RULES = [
  ...
  },
  {
    ...
    evaluate(facts, rule) {
      ...
      return { finding: finding(rule, exposed, refs, `${exposed.length} tool description(s) attempt to control tool selection.`) };
    },
  },
];
...
function helperDigestDescriptors() {
  return [uniqueRefs, finding, coverage, evaluateRules, meetsThreshold, isPromptLintFinding]
    .map(helper => helper.toString().replace(/\r\n?/g, '\n').trim());
}

function createRulesetDigest(evidenceSourceSha256 = RULESET_SOURCES.evidence_sha256, helpers = helperDigestDescriptors()) {
  return sha256({
...
module.exports = { RULES, RULESET_DIGEST, RULESET_VERSION, SEVERITY_ORDER, createRulesetDigest, evaluateRules, isPromptLintFinding, meetsThreshold };
