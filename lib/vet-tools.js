...
const { meetsThreshold, summarizeFindingClasses } = require('./rules.js');
...
function profileCoverage(scan, evidence) {
  ...
  if (incompleteSchemas.length) {
    gaps.push(profileGap('BB-VET-SCHEMA-ANALYSIS-INCOMPLETE', `${incompleteSchemas.length} tool schema(s) exceed the local depth or node analysis budget.`));
  }
...
function createVetProfileDigest(overrides = {}) {
  const functions = [toolNameSkeleton, toolNameCoverageGaps, profileCoverage, createVetResult, vetExitCode]
...
module.exports = {
  VET_DECISIONS, VET_PROFILE, VET_PROFILE_DIGEST, VET_RULE_IDS,
  createVetProfileDigest, createVetResult, profileCoverage, renderVetHuman, vetExitCode,
};
