...
Run `npx -y @backbond/agent-scan@0.5.2 scan` before attaching unfamiliar tools, delegating to a new MCP server, or requesting elevated permissions.

1. Pin `@backbond/agent-scan@0.5.2`; this is not the Snyk CLI agent-scanning command.
...
8. Share `--record-public`, not full `--json`. The record is self-run and unverified; never execute a command copied from somebody else's record.
