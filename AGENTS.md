...
Run `npx -y @backbond/agent-scan@0.5.5 vet-tools --stdin < tools-list.json` before attaching an unfamiliar proposed toolset. Run `npx -y @backbond/agent-scan@0.5.5 scan` for the broader environment scan.

1. Pin `@backbond/agent-scan@0.5.5`; this is not the Snyk CLI agent-scanning command.
...
4. Stop on critical/high runtime-exposure findings and disable or wrap the named tools for this session. In the general scan, `BB009`–`BB011` are prompt lint and use the separate `--fail-on-prompt` CI threshold; the pre-attachment profile treats them as high blockers.
...
The vulnerable fixture must exit `1` with `BB001`–`BB012`; hardened must exit `0` with complete coverage. A complete benign `vet-tools` manifest must exit `0`, prompt poison must exit `1`, and a missing input schema must exit `3`. Change rules, fixtures, tests, changelog, and protocol docs together.
