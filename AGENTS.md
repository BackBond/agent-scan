...
Run `npx -y @backbond/agent-scan@0.6.0 vet-tools --stdin < tools-list.json` before attaching an unfamiliar proposed toolset. Run `npx -y @backbond/agent-scan@0.6.0 scan` for the broader environment scan. In a network-isolated environment, use the operator-staged, checksum-verified `agent-scan.cjs` release asset instead of weakening egress controls.

1. Pin the exact package identity `@backbond/agent-scan@0.6.0`. Never substitute a similarly named package or let a Registry client select an unpinned latest version.
