...
Run `npx -y @backbond/agent-scan@0.5.8 vet-tools --stdin < tools-list.json` before attaching an unfamiliar proposed toolset. Run `npx -y @backbond/agent-scan@0.5.8 scan` for the broader environment scan. In a network-isolated environment, use the operator-staged, checksum-verified `agent-scan.cjs` release asset instead of weakening egress controls.

1. Pin `@backbond/agent-scan@0.5.8`; this is not the Snyk CLI agent-scanning command. Never let a Registry client substitute an unpinned latest version.
