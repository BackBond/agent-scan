...
- Update the public installation pins to `0.6.3`. Published `0.6.2` carries ruleset `backbond-local-rules/2.0.1` and the unfixed BB004 read-only false positive, so the repository tree no longer shares a version string with a differently-behaving published artifact.
...
- Lock the read-only documentation boundary against argument-name sensitivity (BEAAA-28669/F2). `explain_shell`-shaped tools that state they do not execute anything now return the same clean decision whether the single property is named `command` or `text`; both spellings are pinned as corpus-regression boundaries so the verdict can no longer be driven by the property name alone.
