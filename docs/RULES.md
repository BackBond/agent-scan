...
`BB001` and `BB007` distinguish executable interpreter context from generic query, filter, GIS, identifier, and calculator fields. Generic `query`, `code`, or `expression` names require direct action and interpreter context; nearby database prose, a referral code, a quoted executor name, or a reference to another SQL tool is insufficient. Direct shell, runner/executor, Wolfram Language, SQL, HogQL, and other interpreter inputs remain high. Ambiguous unconstrained query/expression fields produce a pre-attachment review gap rather than a high finding. A `const`, nonempty `enum`, or syntactically valid anchored pattern limited to a narrow identifier alphabet constrains execution input. Unanchored, malformed, wildcard, unsafe-range, negated-class, match-any, and length-only patterns do not.

`BB008` likewise requires an active network context and a URL-like unconstrained field. Generic `endpoint`, `href`, `path`, `host`, and `destination` fields produce review gaps. A documentation/reference URL without explicit fetch, download, open, browse, request, or navigation context also routes to review; explicit fetch-the-URL prose remains high. A scheme-only pattern such as `^https://` is not a hostname allowlist; `const`, nonempty `enum`, or a pattern that fixes a hostname can suppress the destination finding. Vendor names or hosts mentioned only in prose do not establish enforcement.
