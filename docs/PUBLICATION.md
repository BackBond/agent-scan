...
3. Review `npm pack --dry-run --json`; the open engine, rules, public-record writer, `server.json`, docs, and fixtures must be present, and release-only network or standalone-build helpers must not be included.
4. Confirm the package version equals the intended immutable release tag.
5. Push the immutable version tag. The protected GitHub workflow publishes the tagged package contents to npm with provenance, re-downloads and content-checks the npm-registry-authoritative tarball, and creates the official GitHub release with those exact npm bytes and their SHA-256 file. It also deterministically builds the unminified, dependency-free `agent-scan.cjs` from the same checkout and publishes that single-file asset with its own SHA-256.
...
7. Install the exact tarball with npm offline mode in a clean directory; rerun the vulnerable and hardened fixture scans, the incomplete coverage gate, the MCP `initialize`/`tools/list` exchange, and a public-record redaction smoke test. Compare the packaged CLI with `agent-scan.cjs` for `scan`, `vet-tools`, and `mcp`, require identical ruleset digests, and run the standalone hardened fixture scan inside a network namespace with no interfaces.
